...
# from visualizer import H

# coarse-level을 제외하곤 이게 먼저 적용된 후에 decoder 인풋으로 들어가짐.
class GaussianFourierFeatureTransform(torch.nn.Module):
    ...
    def forward(self, x):
        x = x.squeeze(0)
        assert x.dim() == 2, 'Expected 2D input (got {}D input)'.format(x.dim()) # 예외처리
        x = x @ self._B.to(x.device)
        return torch.sin(x) #TODO sin함수화!?!
...
class MLP(nn.Module):
    ...
    # 여기서 trilinear interpolation 진행
...
class NICE(nn.Module):
    ...
    def __init__(self, dim=3, c_dim=32,
                 ...
                 color_grid_len=0.16, hidden_size=32, coarse=False, pos_embedding_method='fourier'): # 퓨리에 변환?
        super().__init__()
...
class ILABEL_MLP(nn.Module):
    ...
    def __init__(self, name='', dim=3, c_dim=128,
                 hidden_size=256, n_blocks=5, leaky=False, sample_mode='bilinear',
                 color=False, seg=True, skips=[2], grid_len=0.16, pos_embedding_method='fourier', concat_feature=False):
        super().__init__()
        self.name = name
        self.color = color
        self.no_grad_feature = False
        ...
        if c_dim != 0:
            self.fc_c = nn.ModuleList([
                nn.Linear(c_dim, hidden_size) for i in range(n_blocks) # 여기서 5층으로 만듦.
            ])
        ...
        self.pts_linears = nn.ModuleList(
            [DenseLayer(embedding_size, hidden_size, activation="relu")] +
            [DenseLayer(hidden_size, hidden_size, activation="relu") if i not in self.skips # 3번째 레이어를 키운다.
             else DenseLayer(hidden_size + embedding_size, hidden_size, activation="relu") for i in range(n_blocks-1)])
    ...
    def forward(self, p, c_grid=None):
        ...
        for i, l in enumerate(self.pts_linears):
            ...
            if i in self.skips:
                h = torch.cat([embedded_pts, h], -1)
        out = self.output_linear(h)
        if not self.seg: # 원래 if not self.color
            out = out.squeeze(-1)
        return out

class ILABEL(nn.Module):
    """    
    Segementation based on Neural Implicit Scalable Encoding.
    ...
    Args:
        ...
        fine_grid_len (float): voxel length in fine grid.
        color_grid_len (float): voxel length in color grid.
        seg_grid_len(float): voxel length in seg grid.
        hidden_size (int): hidden size of decoder network
        coarse (bool): whether or not to use coarse level.
        pos_embedding_method (str): positional embedding method.
    """
    ...
    def __init__(self, dim=3, c_dim=32,
                 coarse_grid_len=2.0,  middle_grid_len=0.16, fine_grid_len=0.16,
                 color_grid_len=0.16, seg_grid_len=0.16, hidden_size=32, coarse=False, pos_embedding_method='fourier'): # 퓨리에 변환?
        super().__init__()
        ...
        self.color_decoder = MLP(name='color', dim=dim, c_dim=c_dim, color=True,
                                 skips=[2], n_blocks=5, hidden_size=hidden_size,
                                 grid_len=color_grid_len, pos_embedding_method=pos_embedding_method)
        self.seg_decoder = ILABEL_MLP(name='seg', dim=dim, c_dim=c_dim, color=False,
                                      skips=[2], n_blocks=5, hidden_size=hidden_size,
                                      grid_len=seg_grid_len, pos_embedding_method=pos_embedding_method)

    def forward(self, p, c_grid, stage='middle', **kwargs):
        """
            Output occupancy/color/seg in different stage.
        """
        device = f'cuda:{p.get_device()}'
        if stage == 'coarse':
            occ = self.coarse_decoder(p, c_grid)
