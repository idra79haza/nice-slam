...
class iLabel_plus_SLAM():
    """
    iLabel_plus_SLAM main class.
    Mainly allocate shared resources, and dispatch tracking, mapping and labeling process.
    """
    ...
    def __init__(self, cfg, args):
        ...
        self.cfg = cfg
        self.args = args
        self.ilabel_plus = args.ilabel_plus
        self.nice = args.nice
        ...
        if self.ilabel_plus:
            model = config.get_model(cfg,  ilabel_plus=self.ilabel_plus) # TODO: sample -> raycasting에서 segmentation 정보 추가해야 하나?
        elif self.nice:
            model = config.get_model(cfg,  nice=self.nice) # nice-slam의 conv-onet
        self.shared_decoders = model

        self.scale = cfg['scale']

        self.load_bound(cfg) # 왜하지?!
        if self.ilabel_plus:
        ...
        self.frame_reader = get_dataset(cfg, args, self.scale) # return dataset_dict[replica](cfg, args, scale, device=device) # replica
        ...
        self.labeler = Labeler(cfg, args, self) # TODO: 여기서 추가로 labeling하는 걸 넣는다!? 혹은 Renderer에 그냥 한 번에 한다?!
    ...
    def grid_init(self, cfg):
        ...
        seg_key = 'seg'
        seg_val_shape = list(map(int, (xyz_len/seg_grid_len).tolist()))
        seg_val_shape[0], seg_val_shape[2] = seg_val_shape[2], seg_val_shape[0]
        self.seg_val_shape = seg_val_shape
        val_shape = [1, c_dim, *seg_val_shape]
        seg_val = torch.zeros(val_shape).normal_(mean=0, std=0.01)
        c[seg_key] = seg_val
    ...
    def labeling(self, rank):
        """
        labeling Thread. (updates labeling GT)
    ...
    def run(self):
        ...
        for rank in range(4):
            if rank == 0:
                ...
                p = mp.Process(target=self.labeling, args=(rank, )) ## TODO: 위치가 이게 맞을까!?
            p.start()
            processes.append(p)
        for p in processes:
            p.join()
